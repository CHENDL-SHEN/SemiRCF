...
class EdgeConnect():
    def __init__(self, config):
        ...
        self.out_source_results_path_real = os.path.join(config.PATH, 'results_source_out')
    ...
    def gene_pse(self):
        ...
        for i in names_img_dir:
            ...
            if (not os.path.exists(self.config.TARGET_UNLAB_EDGE_Pseudo_FLIST + '/' + i.replace('.png', '_mask.png'))):
                mask = cv2.imread(self.config.TARGET_UNLAB_MASK_FLIST + '/' + i, 0)
                ones_8 = np.array([-1, -1, -1, -1, 8, -1, -1, -1, -1]).reshape([3, 3])
                ones_tuo = np.ones([self.config.mask_expand, self.config.mask_expand])  #gai6!
                #ones_tuo = np.ones([4, 4])
    ...
    def train(self, semi=True, unsup=False):
        ...
        # pretrained for ResNet optimizator
        if self.config.backebone != 'vgg16' and not unsup:
            ...
            while (pre_epoch < 5):
                ...
                for each_iter in range(totlo_iter):
                    ...
                    logs = self.edge_model.gan_process_step(semi=semi, unsup = unsup, gan=self.config.GAN, mmd=self.config.MMD,
                                                        image=image, edges=edges, mask = None, weight_tar=loss_target_weight / self.config.BATCH_SIZE_MERGE,
                                                        image_source=image_source, edges_source=edges_source, mask_source = None, weight_sou=loss_source_weight / self.config.BATCH_SIZE_MERGE,
                                                        image_test=image_test, edges_test=edges_test, mask_test = None, weight_tar_unlab=loss_unlab_weight / self.config.BATCH_SIZE_MERGE, )

                    if iter_current_target + iter_target_num > total:
                        train_loader_iter = iter(train_loader)
                        iter_current_target = 0
                        flag_train_loader = False
                    ...
                    _, ls_tmp_source = logs[1]
                    _, ls_tmp_unlab = logs[2]
                    _, ls_gen_adr = logs[3]
                    _, ls_dis_adr = logs[4]
                    _, ls_gen_adr_pos = logs[5]
                    _, ls_gen_adr_neg = logs[6]
                    _, ls_dis_adr_pos = logs[7]
                    _, ls_dis_adr_neg = logs[8]
                    _, ls_mmd = logs[9]
                    ...
                    if each_iter % self.config.LOG_INTERVAL == 0 or each_iter == iter_target_num - 1:
                        info_target = 'Pre_Epoch: [{0}/{1}][{2}/{3}] (Tar)'.format(self.config.pretrain_epoch, pre_epoch, each_iter, totlo_iter) + 'Loss tem:{0} (avg:{1}) '.format(ls_tmp_target, totol_loss_target / iter_target_num / (each_iter + 1))
                        info_source = 'Pre_Epoch: [{0}/{1}][{2}/{3}] (Sou)'.format(self.config.pretrain_epoch, pre_epoch, each_iter, totlo_iter) + 'Loss tem:{0} (avg:{1}) '.format(ls_tmp_source, totol_loss_source / iter_source_num / (each_iter + 1))
                        info_unlab = 'Pre_Epoch: [{0}/{1}][{2}/{3}] (Unl)'.format(self.config.pretrain_epoch, pre_epoch, each_iter, totlo_iter) + 'Loss tem:{0} (avg:{1}) '.format(ls_tmp_unlab, totol_loss_unlab / iter_unlab_num / (each_iter + 1))
        ...
        if not unsup:
            loss_target_weight = self.edge_model.loss_weight_tar
        else:
            loss_target_weight = 0
        loss_source_weight = self.edge_model.loss_weight_sou
        if semi:
            loss_unlab_weight = self.edge_model.loss_weight_unlab
        else:
            loss_unlab_weight = 0

        #self.edge_model.iteration = 51
        ...
        while (keep_training):
            ...
            if self.config.EVAL_INTERVAL and (epoch % self.config.EVAL_INTERVAL == 0) or (epoch == 1):
                print('\nstart eval...\n')
                self.eval(unsup)

            if self.config.SAMPLE_TEST and (epoch % self.config.SAMPLE_TEST == 0) or (epoch == 1):
                print('\nstart test...\n')
                self.real_test_target_out(unsup)
            ...
            totol_loss_source = 0
            totol_loss_unlab = 0
            totol_loss_gen_adr_pos = 0
            totol_loss_gen_adr_neg = 0
            totol_loss_dis_adr_pos = 0
            totol_loss_dis_adr_neg = 0
            totol_loss_gen = 0
            totol_loss_dis = 0
            flag_train_loader = True
            # gai
            #if not unsup and not semi:
            #    loss_target_weight = loss_source_weight
            ...
            for each_iter in range(totlo_iter):
                ...
                totol_loss_gen = totol_loss_gen + ls_gen_adr
                totol_loss_dis = totol_loss_dis + ls_dis_adr
                totol_loss_target = totol_loss_target + ls_tmp_target
                totol_loss_source = totol_loss_source + ls_tmp_source
                totol_loss_unlab = totol_loss_unlab + ls_tmp_unlab
                totol_loss_gen_adr_pos = totol_loss_gen_adr_pos + ls_gen_adr_pos
                totol_loss_gen_adr_neg = totol_loss_gen_adr_neg + ls_gen_adr_neg
                totol_loss_dis_adr_pos = totol_loss_dis_adr_pos + ls_dis_adr_pos
                totol_loss_dis_adr_neg = totol_loss_dis_adr_neg + ls_dis_adr_neg
                totol_loss_mmd = totol_loss_mmd + ls_mmd

                if not unsup:
                    loss_target_weight = (totol_loss_target + totol_loss_source + totol_loss_unlab) * self.config.target_lab_weight / (self.config.target_lab_weight + self.config.source_lab_weight + self.config.target_unlab_weight) / (
                        totol_loss_target)
                else:
                    loss_target_weight = 0
                loss_source_weight = (totol_loss_target + totol_loss_source + totol_loss_unlab) * self.config.source_lab_weight / (self.config.target_lab_weight + self.config.source_lab_weight + self.config.target_unlab_weight) / (
                    totol_loss_source)
                if semi and totol_loss_unlab > 0:
                    loss_unlab_weight = (totol_loss_target + totol_loss_source + totol_loss_unlab) * self.config.target_unlab_weight / (self.config.target_lab_weight + self.config.source_lab_weight + self.config.target_unlab_weight) / (
                        totol_loss_unlab)
                else:
                    loss_unlab_weight = 0
                ...
                # gai
                #if not unsup and not semi:
                #    if not flag_train_loader:
                #        loss_target_weight = 0

                if each_iter % self.config.LOG_INTERVAL == 0 or each_iter == iter_target_num - 1:
                    info_target = 'Epoch: [{0}/{1}][{2}/{3}] (Tar)'.format(epoch, self.MAX_ITERS, each_iter, totlo_iter) + \
                                  'Loss tem:{0} (avg:{1}) '.format(ls_tmp_target, totol_loss_target / iter_target_num / (each_iter + 1))
                    info_source = 'Epoch: [{0}/{1}][{2}/{3}] (Sou)'.format(epoch, self.MAX_ITERS, each_iter, totlo_iter) + \
                                  'Loss tem:{0} (avg:{1}) '.format(ls_tmp_source, totol_loss_source / iter_source_num / (each_iter + 1))
                    info_unlab = 'Epoch: [{0}/{1}][{2}/{3}] (Unl)'.format(epoch, self.MAX_ITERS, each_iter, totlo_iter) + \
                                 'Loss tem:{0} (avg:{1}) '.format(ls_tmp_unlab, totol_loss_unlab / iter_unlab_num / (each_iter + 1))
                    info_GEN = 'Epoch: [{0}/{1}][{2}/{3}] (GEN)'.format(epoch, self.MAX_ITERS, each_iter, totlo_iter) + \
                               'Loss GEN_tem:{0}(Pos:{1:.3f}+Neg:{2:.3f}) (avg:{3}(avg_pos:{4:.3f}+avg_neg:{5:.3f})) '\
                                   .format(ls_gen_adr, ls_gen_adr_pos, ls_gen_adr_neg, totol_loss_gen / (each_iter + 1), totol_loss_gen_adr_pos / (each_iter + 1), totol_loss_gen_adr_neg / (each_iter + 1))
                    info_DIS = 'Epoch: [{0}/{1}][{2}/{3}] (DIS)'.format(epoch, self.MAX_ITERS, each_iter, totlo_iter) + \
                               'Loss DIS_tem:{0}(Pos:{1:.3f}+Neg:{2:.3f}) (avg:{3}(avg_pos:{4:.3f}+avg_neg:{5:.3f})) '\
                                   .format(ls_dis_adr, ls_dis_adr_pos, ls_dis_adr_neg, totol_loss_dis / (each_iter + 1), totol_loss_dis_adr_pos / (each_iter + 1), totol_loss_dis_adr_neg / (each_iter + 1))
                    info_MMD = 'Epoch: [{0}/{1}][{2}/{3}] (MMD)'.format(epoch, self.MAX_ITERS, each_iter, totlo_iter) + \
                               'Loss MMD_tem:{0} (avg:{1}) '.format(ls_mmd, totol_loss_mmd / (each_iter + 1))
                    print('loss_target_weight: ' + str(loss_target_weight) + 'loss_source_weight: ' + str(loss_source_weight) + ' loss_unlab_weight: ' + str(loss_unlab_weight))
                    print(info_target)
                    print(info_source)
                    print(info_unlab)
                    ...
                    if self.config.GAN:
                        print(info_GEN)
                        print(info_DIS)
                        self.log(info_GEN, True)
                        self.log(info_DIS, True)
                    if self.config.MMD:
                        print(info_MMD)
                        self.log(info_MMD, True)
                    #break


            # save loss weight
            self.edge_model.loss_weight_tar = loss_target_weight
            self.edge_model.loss_weight_sou = loss_source_weight
            self.edge_model.loss_weight_unlab = loss_unlab_weight
            ...
            if epoch <= self.config.epoch_tune and semi:
                ...
                for idx_batch, items_source in enumerate(train_unlab_pseudo_dataset_each):
                    ...
                    print(info_unlab)
                    if not unsup:
                        self.edge_model.medium_feature(image_test, edges_test, image_name, mode=3, tune_flag=tune_flag, epoch=epoch,
                                                       arlfa = self.config.arlfa, beta = self.config.beta, gema = self.config.gema)
                    else:
                        self.edge_model.medium_feature(image_test, edges_test, image_name, mode=3, tune_flag=tune_flag, epoch=epoch,
                                                       arlfa = 0, beta = self.config.beta_unsup, gema = self.config.gema_unsup)
                    # self.edge_model.gen_optimizer.zero_grad()

                    if '\\' in image_name[0]:  # for windows
                        name_lst.append((epoch, image_name[0].split('\\')[-1], 
                                         self.config.mask_ratio, self.config.region_min,
    ...
    def eval(self, unsup = False):
        # 如果是预训练+双边预测可以用这个
        if self.config.MODE == 3 and unsup:
            unsup = True
        else:
            unsup = False
    ...
    def real_test_target_out(self, unsup = False):
        # 如果是预训练+双边预测可以用这个
        if self.config.MODE == 3 and unsup:
            unsup = True
        else:
        ...
        for items in out_test_loader:
            ...
            if not unsup:
                ...
                _, output, _ = self.edge_model(image_test)
    ...
    def real_test_out(self, domain='target', unsup = False):
        # 如果是预训练+双边预测可以用这个
        if self.config.MODE == 3 and unsup:
            unsup = True
        else:
            unsup = False
        self.edge_model.eval()
        if not os.path.exists(self.out_results_path_real) and domain == 'target':
            os.makedirs(self.out_results_path_real)
        if not os.path.exists(self.out_source_results_path_real) and domain == 'source':
            os.makedirs(self.out_source_results_path_real)
        ...
        for items in out_test_loader:
            ...
            # edge model
            if domain == 'target' and not unsup:
                output, _, _ = self.edge_model(image_test)
            elif domain == 'source' or unsup:
                _, output, _ = self.edge_model(image_test)
            ...
            if domain == 'target':
                ...
                path = os.path.join(self.out_source_results_path_real, name)
            images.save(path)
