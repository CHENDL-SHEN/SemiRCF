...
from .networks import EdgeGenerator_VGG16, EdgeGeneratorRES50, EdgeGeneratorRES101, EdgeGeneratorRESNEXT50, EdgeGeneratorRESNEXT101, Discriminator
from .loss import cross_entropy_loss_RCF, cross_entropy_loss_perued_RCF, AdversarialLoss, MMD_loss
...
def load_res50pretrain(model, Res50='ImageNet/resnet50-19c8e357.pth'):
    mapping_block = {'layer1.0' : 'conv2a', 'layer1.1' : 'conv2b', 'layer1.2' : 'conv2c',
                     'layer2.0' : 'conv3a', 'layer2.1' : 'conv3b', 'layer2.2' : 'conv3c', 'layer2.3' : 'conv3d',
                     'layer3.0' : 'conv4a', 'layer3.1' : 'conv4b', 'layer3.2' : 'conv4c', 'layer3.3' : 'conv4d', 'layer3.4' : 'conv4e', 'layer3.5' : 'conv4f',
                     'layer4.0' : 'conv5a', 'layer4.1' : 'conv5b', 'layer4.2' : 'conv5c'}
    mapping_first = {'conv1.weight' : 'conv1.0.weight',
    ...
    torch_params_raw = torch.load(Res50)
    torch_params_tar =  model.state_dict()
    for k in torch_params_raw:
        if k in mapping_first.keys():
            torch_params_tar[mapping_first[k]] = torch_params_raw[k]
        else:
            for mp_key in mapping_block.keys():
                if mp_key in k and (not 'bn' in k) and ('downsample' not in k):
                    k_real = k.replace(mp_key, mapping_block[mp_key])
                    torch_params_tar[k_real] = torch_params_raw[k]

    model.load_state_dict(torch_params_tar)
    print('loading RES50')

def load_resx50pretrain(model, Resx50='ImageNet/resnext50_32x4d-7cdf4587.pth'):
    mapping_block = {'layer1.0' : 'conv2a', 'layer1.1' : 'conv2b', 'layer1.2' : 'conv2c',
                     'layer2.0' : 'conv3a', 'layer2.1' : 'conv3b', 'layer2.2' : 'conv3c', 'layer2.3' : 'conv3d',
                     'layer3.0' : 'conv4a', 'layer3.1' : 'conv4b', 'layer3.2' : 'conv4c', 'layer3.3' : 'conv4d', 'layer3.4' : 'conv4e', 'layer3.5' : 'conv4f',
                     'layer4.0' : 'conv5a', 'layer4.1' : 'conv5b', 'layer4.2' : 'conv5c'}
    ...
    torch_params_raw = torch.load(Resx50)
    torch_params_tar =  model.state_dict()
    for k in torch_params_raw:
        if k in mapping_first.keys():
            torch_params_tar[mapping_first[k]] = torch_params_raw[k]
            ...
            for mp_key in mapping_block.keys():
                if mp_key in k and (not 'bn' in k) and ('downsample.1' not in k):
                    k_real = k.replace(mp_key, mapping_block[mp_key])
                    torch_params_tar[k_real] = torch_params_raw[k]

    model.load_state_dict(torch_params_tar)
    print('loading RESNEXT50')

def load_res101pretrain(model, Res101='ImageNet/resnet101-5d3b4d8f.pth'):
    mapping_block = {'layer1.0' : 'conv2a', 'layer1.1' : 'conv2b', 'layer1.2' : 'conv2c',
                     'layer2.0' : 'conv3a', 'layer2.1' : 'conv3b', 'layer2.2' : 'conv3c', 'layer2.3' : 'conv3d',
                     'layer3.0.' : 'conv4a.', 'layer3.1.' : 'conv4b.', 'layer3.2.' : 'conv4c.', 'layer3.3.' : 'conv4d.', 'layer3.4.' : 'conv4e.', 'layer3.5.' : 'conv4f.',
                     'layer3.6.' : 'conv4g.', 'layer3.7.' : 'conv4h.', 'layer3.8.' : 'conv4i.', 'layer3.9.' : 'conv4j.', 'layer3.10.': 'conv4k.', 'layer3.11.': 'conv4l.',
                     'layer3.12.': 'conv4m.', 'layer3.13.': 'conv4n.', 'layer3.14.': 'conv4o.', 'layer3.15.': 'conv4p.', 'layer3.16.': 'conv4q.', 'layer3.17.': 'conv4r.',
                     'layer3.18.': 'conv4s.', 'layer3.19.': 'conv4t.', 'layer3.20.': 'conv4u.', 'layer3.21.': 'conv4v.', 'layer3.22.': 'conv4w.',
                     'layer4.0' : 'conv5a', 'layer4.1' : 'conv5b', 'layer4.2' : 'conv5c'}
    mapping_first = {'conv1.weight' : 'conv1.0.weight',
                     #'bn1.weight' :'conv1.1.weight',
                     #'bn1.running_mean': 'conv1.1.running_mean',
    ...
    torch_params_raw = torch.load(Res101)
    torch_params_tar =  model.state_dict()
    for k in torch_params_raw:
        if k in mapping_first.keys():
            torch_params_tar[mapping_first[k]] = torch_params_raw[k]
    ...
    model.load_state_dict(torch_params_tar)
    print('loading RES101')

def load_resx101pretrain(model, Resx101='ImageNet/resnext101_32x8d-8ba56ff5.pth'):
    mapping_block = {'layer1.0' : 'conv2a', 'layer1.1' : 'conv2b', 'layer1.2' : 'conv2c',
                     'layer2.0' : 'conv3a', 'layer2.1' : 'conv3b', 'layer2.2' : 'conv3c', 'layer2.3' : 'conv3d',
                     'layer3.0.' : 'conv4a.', 'layer3.1.' : 'conv4b.', 'layer3.2.' : 'conv4c.', 'layer3.3.' : 'conv4d.', 'layer3.4.' : 'conv4e.', 'layer3.5.' : 'conv4f.',
                     'layer3.6.' : 'conv4g.', 'layer3.7.' : 'conv4h.', 'layer3.8.' : 'conv4i.', 'layer3.9.' : 'conv4j.', 'layer3.10.': 'conv4k.', 'layer3.11.': 'conv4l.',
    ...
    torch_params_raw = torch.load(Resx101)
    torch_params_tar =  model.state_dict()
    for k in torch_params_raw:
        if k in mapping_first.keys():
            torch_params_tar[mapping_first[k]] = torch_params_raw[k]
...
class BaseModel(nn.Module):
    def __init__(self, name, config):
        ...
        self.config = config
        self.iteration = 0
        self.loss_weight_tar = 1
        self.loss_weight_sou = 1
        self.loss_weight_unlab = 1
    ...
    def load(self):
        if os.path.exists(self.gen_weights_path):
            ...
            if 'RCF-pytorch-master' in self.gen_weights_path:
                ...
                if self.config.MODE == 1 and self.config.data_mode == "gray" :  # 重构无监督下赋值对于金相图像
                    data['generator']['score_dsn1.weight'] = data['generator']['score_dsn1s.weight']
                    data['generator']['score_dsn2.weight'] = data['generator']['score_dsn2s.weight']
                    data['generator']['score_dsn3.weight'] = data['generator']['score_dsn3s.weight']
                    data['generator']['score_dsn4.weight'] = data['generator']['score_dsn4s.weight']
                    data['generator']['score_dsn5.weight'] = data['generator']['score_dsn5s.weight']
                    data['generator']['score_dsn1.bias'] = data['generator']['score_dsn1s.bias']
                    data['generator']['score_dsn2.bias'] = data['generator']['score_dsn2s.bias']
                    data['generator']['score_dsn3.bias'] = data['generator']['score_dsn3s.bias']
                    data['generator']['score_dsn4.bias'] = data['generator']['score_dsn4s.bias']
                    data['generator']['score_dsn5.bias'] = data['generator']['score_dsn5s.bias']
                    data['generator']['score_final.weight'] = data['generator']['score_finals.weight']
                    data['generator']['score_final.bias'] = data['generator']['score_finals.bias']
                    print('loading: old 12: overrid score layer')
                self.generator.load_state_dict(data['generator'])
                if 'gen_optimizer' in data.keys():
                    self.gen_optimizer.load_state_dict(data['gen_optimizer'])
                    print('Lodaed generator & gen_optimizer')
                else:
                    print('Lodaed generator only')
                if self.config.GAN:
                    if 'discriminator' in data.keys():
                        self.discriminator.load_state_dict(data['discriminator'])
                        if 'gen_optimizer' in data.keys():
                            self.dis_optimizer.load_state_dict(data['dis_optimizer'])
                            print('Lodaed discriminator & dis_optimizer')
                        else:
                            print('Lodaed discriminator only')

            if 'loss_weight_tar' in data.keys():
                self.loss_weight_tar = data['loss_weight_tar']
                self.loss_weight_sou = data['loss_weight_sou']
                self.loss_weight_unlab = data['loss_weight_unlab']
                print('Lodaed Loss weight')
            ...
            if self.config.backebone == 'vgg16':
                load_vgg16pretrain(self.generator)
            elif self.config.backebone == 'resnet50':
                load_res50pretrain(self.generator)
            elif self.config.backebone == 'resnext50':
                load_resx50pretrain(self.generator)
            elif self.config.backebone == 'resnet101':
                load_res101pretrain(self.generator)
            elif self.config.backebone == 'resnext101':
                load_resx101pretrain(self.generator)
    ...
    def save(self):
        ...
        if self.config.GAN:
            torch.save({
                ...
                'loss_weight_tar': self.loss_weight_tar,
                'loss_weight_sou': self.loss_weight_sou,
                'loss_weight_unlab': self.loss_weight_unlab,
            }, os.path.join(self.config.PATH, self.name + '_' + self.config.backebone + '_' + str(self.iteration) + '_' + '_gen.pth'))
        else:
            torch.save({
                'iteration': self.iteration,
                'generator': self.generator.state_dict(),
...
def build_label_process(arg):
    ...
    def dele_mask_bound(edge, mask, mask_ratio):
        def getGrayDiff(img, currentPoint, tmpPoint):
            return img[currentPoint.x, currentPoint.y] * img[tmpPoint.x, tmpPoint.y]
        ...
        for i in range(s1.shape[0]):
            ...
            y = s2[i]
            seedmark = regionGrow(iso_point_grow, nbeibour_1, Point(x, y), label=label)
            num_mask = np.where(np.logical_and(seedmark == label, mask == 255) == True)[0].shape[0]
            num_total = np.where(seedmark == label)[0].shape[0]
            if num_mask > num_total * mask_ratio:
                edge[np.where(seedmark == label)] = muhu_value
            label = label + 1
        return edge


    epoch_globel, image_name, mask_ratio, region_min, region_min_second, region_min_ratio, edge_dele_ratio, edge_dele_num, threhold_, \
    ...
    positive_forcp = positive.copy()
    negtive_forcp = negtive.copy()
    ...
    if neg_num_raw != 0:
        pos_neg_ratio_tmp = pos_num_raw / neg_num_raw
        if pos_neg_ratio_tmp > 0.055:
            flag_w = 'P'
        elif pos_neg_ratio_tmp < 0.035:
            flag_w = 'N'
    ...
    if data_mode == 'rgb':
        ...
        while (num < pos_need_num):
            x_a, y_b = np.unravel_index(np.argmax(positive_advance), positive_advance.shape)
            # if positive_advance[x_a, y_b] <= threshold_p: gai1 !! limit pos generate
            if positive_advance[x_a, y_b] <= threshold_p or (edges[x_a, y_b] == 0 and positive_advance[x_a, y_b] <= threhold_fugai):
    ...
    #edges_con = dele_mask_bound(edges.copy(), mask, mask_ratio)
    ...
    #if (num_totla_pos < pos_need_num * 0.5 or tune_flag) and epoch_globel > epoch_could_pos_tune:  # raw
    #if (num_totla_pos < pos_need_num * 0.5 or tune_flag) and epoch_globel > epoch_could_pos_tune and Cound_fine:  # gai2!
    if (abs(num_totla_pos) < abs(pos_need_num * 0.5) or tune_flag) and epoch_globel > epoch_could_pos_tune:  # gai4!

        ## gai5 !!  说明越修改越完
        edges_con_cp_used = edges_con.copy()
        pos_num_in = np.where(edges_con_cp_used[pad_num:height + pad_num, pad_num:width + pad_num] == 1)[0].shape[0]
        ...
        ## gai5 !!  说明越修改越完
        pos_num_new = np.where(edges_con[pad_num:height + pad_num, pad_num:width + pad_num] == 1)[0].shape[0]
        num_totla_pos = pos_num_new - pos_num_in
        if (num_totla_pos > 0 and flag_w == 'P') or (num_totla_pos < 0 and flag_w == 'N') :
            flag_t = 'E'
            edges_con = edges_con_cp_used
    ...
    name_new_tmp = UNLAB_TRAIN_EDGE_Pseudo_FLIST + '/' + image_name.split('.')[0] + 'epoch_' + str(epoch_globel) + '_' + flag_t + flag_e + flag_w + '_' + str(num_totla_pos) + '__' + str(num_totla_neg) + '.png'
...
def build_label_process(arg):
    ...
    def get_advance_mask(edges, positive, mask):
        ...
        positive_advance[np.where(mask == 255)] = 0
        # fixed
        positive_advance[pad_num, :] = 0
        positive_advance[-pad_num - 1, :] = 0
        positive_advance[:, pad_num] = 0
        positive_advance[:, -pad_num - 1] = 0
        # fixed-over
        positive_advance, shuxing_advance = gene_point(positive_advance, edges.copy())
    ...
    def iso_point(edges):
        ...
        region = signal.correlate2d(edges, ones, boundary='fill', mode='same')
        #bound = signal.correlate2d(edges_cp, np.ones([9, 9]), boundary='fill', mode='same')
        bound = signal.correlate2d(edges_cp, np.ones([3, 3]), boundary='fill', mode='same')
        n1, n2, n3, n4, n5, n6, n7, n8 = filt_0(edges)
    ...
    # fixed
    def dele_frame_bound(edge, epoch_globel, epoch_could_tune):
        pad_inter = np.ones(edge.shape)
        pad_inter[pad_num + 1:-pad_num-2, pad_num + 1:-pad_num-2] = 0

        if epoch_globel > epoch_could_tune:
            fill_value = 0
        else:
            fill_value = muhu_value
        # dele frame bound
        edge_cp = edge.copy()
        ones_1 = np.array([0, 1, 0, 0, 1, 0, 0, 1, 0]).reshape([3, 3])
        ones_2 = np.array([0, 0, 0, 1, 1, 1, 0, 0, 0]).reshape([3, 3])
        edge[np.where(edge == -1)] = 100
        nbeibour_1 = signal.correlate2d(edge, ones_1, boundary='fill', mode='same')
        nbeibour_2 = signal.correlate2d(edge, ones_2, boundary='fill', mode='same')
        edge[pad_num,pad_num:-pad_num-1] = -3
        edge[-pad_num-1,pad_num:-pad_num-1] = -3
        edge[pad_num:-pad_num-1,pad_num] = -2
        edge[pad_num:-pad_num-1,-pad_num-1] = -2
        edge_cp[np.logical_and(edge == -3, nbeibour_1 !=102)] = fill_value
        edge_cp[np.logical_and(edge == -2, nbeibour_2 != 102)] = fill_value
        edge_cp[pad_num,pad_num] = fill_value
        edge_cp[pad_num,-pad_num-1] = fill_value
        edge_cp[-pad_num-1,pad_num] = fill_value
        edge_cp[-pad_num-1,-pad_num-1] = fill_value

        # dele frame grown bound
        edge_num = np.where(edge_cp == 1)[0].shape[0]
        max_dele_num = min(int(edge_num * edge_dele_ratio), edge_dele_num)
        s1, s2 = np.where(np.logical_and(edge == -3, edge_cp == 1)==True)
        s1s, s2s = np.where(np.logical_and(edge == -2, edge_cp == 1)==True)
        label = 5
        for i in range(s1.shape[0]):
            x = s1[i]
            y = s2[i]
            seedmark = regionGrow(edge_cp.copy(), Point(x, y), label=label)
            frame_num_seed = len(np.where(np.logical_and(seedmark == label, pad_inter == 1) == True)[0])
            if frame_num_seed >= 2:
                max_dele_num_u = max_dele_num * 2
                #max_dele_num_u = max_dele_num
            else:
                max_dele_num_u = max_dele_num

            if np.where(seedmark == label)[0].shape[0] < max_dele_num_u:
                edge_cp[np.where(seedmark == label)] = fill_value
            label = label + 1
        for i in range(s1s.shape[0]):
            x = s1s[i]
            y = s2s[i]
            seedmark = regionGrow(edge_cp.copy(), Point(x, y), label=label)
            if np.where(seedmark == label)[0].shape[0] < max_dele_num:
    ...
    def pos_adv_add_frame(positive_advance, x_a, y_b, shuxing_advance, threhold_pos_adv):
        shuxing_advance[x_a + 1, y_b] = -1
        shuxing_advance[x_a, y_b - 1] = -1
        shuxing_advance[x_a, y_b + 1] = -1
        shuxing_advance[x_a - 1, y_b] = -1
    ...
    pos_num_raw = np.where(edges == 1)[0].shape[0]
    neg_num_raw = np.where(edges == 0)[0].shape[0]
    if neg_num_raw != 0 and data_mode == 'gray':
        pos_neg_ratio_tmp = pos_num_raw / neg_num_raw
        if pos_neg_ratio_tmp > 0.055:
            flag_w = 'P'
        elif pos_neg_ratio_tmp < 0.035:
    ...
    elif neg_num_raw != 0 and data_mode == 'rgb':
        pos_neg_ratio_tmp = pos_num_raw / neg_num_raw
        if pos_neg_ratio_tmp > 0.060:
            flag_w = 'P'
        elif pos_neg_ratio_tmp < 0.025:
            flag_w = 'N'
    ...
    if data_mode == 'rgb':
        ...
        while (num < pos_need_num):
            x_a, y_b = np.unravel_index(np.argmax(positive_advance), positive_advance.shape)
            # if positive_advance[x_a, y_b] <= threshold_p: gai1 !! limit pos generate 不成功的
            #if positive_advance[x_a, y_b] <= threshold_p or (edges[x_a, y_b] == 0 and positive_advance[x_a, y_b] <= threhold_fugai):
            if positive_advance[x_a, y_b] <= threshold_p:
                nbeibour_7 = pos_neibour(edges.copy(), kernal=int(min_distance), method='dis')
                positive_cp = positive.copy()
            ...
            if edges[x_a, y_b] != 1:
                edges[x_a, y_b] = 1
                if x_a == pad_num or x_a == height + pad_num - 1 or y_b == pad_num or y_b == pad_num + width - 1:
                    positive_advance[x_a, y_b] = threshold_p
                    nbeibour = pos_neibour_add(nbeibour, x_a, y_b)
                    num = num + 1
                    positive_advance, shuxing_advance = pos_adv_add_frame(positive_advance, x_a, y_b, shuxing_advance, 0)
                    continue
    ...
    edges_con = dele_cornor(edges_con.copy())
    edges_con = hard_connection(edges_con.copy(), hard_distance)
    edges_con = dele_frame_bound(edges_con.copy(),epoch_globel, epoch_could_tune)
    ...
    name_new_neg_expand_epoch = UNLAB_TRAIN_EDGE_Pseudo_FLIST + '/' + image_name.split('.')[0] + 'epoch_' + str(epoch_globel) + '_neg_expand.png'
    pos_epoch = UNLAB_TRAIN_EDGE_Pseudo_FLIST + '/' + image_name.replace('.png', 'epoch_' + str(epoch_globel) + '_pos.npy')
    neg_epoch = UNLAB_TRAIN_EDGE_Pseudo_FLIST + '/' + image_name.replace('.png', 'epoch_' + str(epoch_globel) + '_neg.npy')
    np.save(pos_epoch, positive_forcp)  ## lmc for windows
    np.save(neg_epoch, negtive_forcp)  ## lmc for windows
    cv2.imwrite(name_new_neg_expand_epoch, neg_expand)
    '''
    print(name_new + ' is OK! :' + str(num_totla_pos) + '__' + str(num_totla_neg))
...
class bank_fea():
    ...
    def build_label(self, edges, fes_medium_, image_name, epoch, arlfa, beta, gema):
        ...
        active_hot_map_neg = active_hot_map_neg * arlfa + active_hot_map_neg_source * beta
        active_hot_map_pos = active_hot_map_pos * arlfa + active_hot_map_pos_source * beta
        ...
        if len(neg_represent) > 0 and len(pos_represent) > 0:
            ...
            active_hot_map_neg = active_hot_map_neg + active_hot_map_neg_test * gema
            active_hot_map_pos = active_hot_map_pos + active_hot_map_pos_test * gema
...
class EdgeModel(BaseModel):
    def __init__(self, config):
        ...
        if config.backebone == 'vgg16':
            generator = EdgeGenerator_VGG16()
        elif config.backebone == 'resnet50':
            generator = EdgeGeneratorRES50()
        elif config.backebone == 'resnet101':
            generator = EdgeGeneratorRES101()
        elif config.backebone == 'resnext50':
            generator = EdgeGeneratorRESNEXT50()
        elif config.backebone == 'resnext101':
            generator = EdgeGeneratorRESNEXT101()
        ...
        if config.GAN:
            discriminator = Discriminator(in_channels= 105)
            l1_loss = nn.MSELoss()
        ...
        if config.MMD:
            mmd_loss = MMD_loss()
            self.add_module('mmd_loss', mmd_loss)
    ...
    def calculate_gradient_penalty(self, real_images, fake_images):
        ...
        grad_penalty = ((gradients.norm(2, dim=1) - 1) ** 2).mean() * self.config.lambda_term
        return grad_penalty


    def medium_feature(self, images, edges, images_name = None,  mode=1, tune_flag = False, epoch = 1000, arlfa = 0, beta = 0, gema = 0):
        ...
        if mode == 1:
            ...
            self.bank_fea.build_label(edges.detach(), fes_medium.detach(), images_name, epoch = self.iteration, arlfa = arlfa, beta = beta, gema = gema)  #edge_lab_fuzzy

        return



    def gan_process_step(self, semi = True, unsup = False, gan = False, mmd = False,
                         image = None, edges = None, mask = None, weight_tar = None,
                         image_source = None, edges_source = None, mask_source = None, weight_sou = None,
                         image_test = None, edges_test = None, mask_test = None, weight_tar_unlab = None):

        target_outputs_lab, _, fea_fake_lab = self(image)
        _, source_outputs, fea_real = self(image_source)
        target_outputs_unlab, _, fea_fake_unlab = self(image_test)


        tar_gen_bce_loss = 0
        sou_gen_bce_loss = 0
        unlab_gen_bce_loss = 0
        lmc_dis_loss = 0
        lmc_gen_loss = 0
        lmc_mmd_loss = 0

        if not unsup:
            for idxx, o in enumerate(target_outputs_lab):
                loss_tmp, _ = cross_entropy_loss_RCF(o, edges)
                tar_gen_bce_loss += loss_tmp
            tar_gen_bce_loss_wei = tar_gen_bce_loss * weight_tar

        for idxx, o in enumerate(source_outputs):
            loss_tmp, _ = cross_entropy_loss_RCF(o, edges_source)
            sou_gen_bce_loss += loss_tmp
        sou_gen_bce_loss_wei = sou_gen_bce_loss * weight_sou

        if semi:
            for idxx, o in enumerate(target_outputs_unlab):
                loss_tmp = cross_entropy_loss_perued_RCF(o, edges_test)
                unlab_gen_bce_loss += loss_tmp
            unlab_gen_bce_loss_wei = unlab_gen_bce_loss * weight_tar_unlab

        # discriminator loss
        fea_fake = torch.cat((fea_fake_lab, fea_fake_unlab), dim=0)
        ...
        if gan:
            ...
            for d_iter in range(self.config.critic_iter):
                self.dis_optimizer.zero_grad()
                fake_fea_dice = random.sample(range(0, fea_fake.shape[0] * fea_fake.shape[2] * fea_fake.shape[3]), self.config.GAN_sample)
                real_fea_dice = random.sample(range(0, fea_real.shape[0] * fea_real.shape[2] * fea_real.shape[3]), self.config.GAN_sample)
                one = torch.tensor(1, dtype=torch.float).cuda()
                mone = one * -1

                real_fea_hotwise = fea_real.permute(0, 2, 3, 1).reshape([fea_real.shape[0] * fea_real.shape[2] * fea_real.shape[3], 105])[real_fea_dice]
                fake_fea_hotwise = fea_fake.permute(0, 2, 3, 1).reshape([fea_fake.shape[0] * fea_fake.shape[2] * fea_fake.shape[3], 105])[fake_fea_dice]
                dis_real = self.discriminator(real_fea_hotwise.detach())  # in: (grayscale(1) + edge(1))
                dis_fake = self.discriminator(fake_fea_hotwise.detach())  # in: (grayscale(1) + edge(1))
                dis_real = dis_real.mean()
                dis_fake = dis_fake.mean()
                dis_real.backward(mone * self.config.GAN_weight)
                dis_fake.backward(one * self.config.GAN_weight)

                # Train with gradient penalty
                gradient_penalty = self.calculate_gradient_penalty(real_fea_hotwise, fake_fea_hotwise)
                gradient_penalty.backward(one * self.config.GAN_weight)
                lmc_dis_loss = dis_fake - dis_real + gradient_penalty
                Wasserstein_D = dis_real - dis_fake
            ...
            gen_real = self.discriminator(real_fea_hotwise)  # in: (grayscale(1) + edge(1))
            gen_fake = self.discriminator(fake_fea_hotwise)  # in: (grayscale(1) + edge(1))
            gen_real = gen_real.mean()
            gen_fake = gen_fake.mean()

            lmc_gen_loss += (gen_real + gen_fake)
            lmc_gen_loss_wei = self.config.GAN_weight * lmc_gen_loss

        if mmd:
            fake_fea_dice = random.sample(range(0, fea_fake.shape[0] * fea_fake.shape[2] * fea_fake.shape[3]), self.config.MMD_sample)
            real_fea_dice = random.sample(range(0, fea_real.shape[0] * fea_real.shape[2] * fea_real.shape[3]), self.config.MMD_sample)
            fake_fea_hotwise = fea_fake.permute(0, 2, 3, 1).reshape([fea_fake.shape[0] * fea_fake.shape[2] * fea_fake.shape[3], 105])[fake_fea_dice]
            real_fea_hotwise = fea_real.permute(0, 2, 3, 1).reshape([fea_real.shape[0] * fea_real.shape[2] * fea_real.shape[3], 105])[real_fea_dice]
            lmc_mmd_loss += self.mmd_loss(fake_fea_hotwise, real_fea_hotwise)
            lmc_mmd_loss_wei = self.config.MMD_weight * lmc_mmd_loss
        ...
        logs = []
        if not unsup:
            tar_gen_bce_loss_wei.backward(retain_graph=True)
            logs.append(("l_bce_tar", tar_gen_bce_loss.item()))
        else:
            logs.append(("l_bce_tar", 0))

        sou_gen_bce_loss_wei.backward(retain_graph=True)
        logs.append(("l_bce_sou", sou_gen_bce_loss.item()))

        if semi:
            unlab_gen_bce_loss_wei.backward(retain_graph=True)
            logs.append(("l_bce_tar_unlab", unlab_gen_bce_loss.item()))
        else:
            logs.append(("l_bce_tar_unlab", 0))

        if gan:
            logs.append(("l_gen_adr", lmc_gen_loss.item()))
            logs.append(("l_dis_adr", lmc_dis_loss.item()))
            logs.append(("l_gen_adr_pos", gen_real.item()))
            logs.append(("l_gen_adr_neg", gen_fake.item()))
            logs.append(("l_dis_adr_pos", dis_real.item()))
            logs.append(("l_dis_adr_neg", dis_fake.item()))
            gen_real.backward(one * self.config.GAN_weight,retain_graph=True)
            gen_fake.backward(mone * self.config.GAN_weight,retain_graph=True)
        else:
            logs.append(("l_gen_adr", 0))
            logs.append(("l_dis_adr", 0))
            logs.append(("l_gen_adr_pos", 0))
            logs.append(("l_gen_adr_neg", 0))
            logs.append(("l_dis_adr_pos", 0))
            logs.append(("l_dis_adr_neg", 0))

        if mmd:
            lmc_mmd_loss_wei.backward(retain_graph=True)
            logs.append(("l_mmd", lmc_mmd_loss.item()))
        else:
            logs.append(("l_mmd", 0))
